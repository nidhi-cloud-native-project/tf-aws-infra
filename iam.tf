    ...
    Statement = [
      {
        ...
        Resource = "arn:aws:secretsmanager:us-east-1:${data.aws_caller_identity.current.account_id}:secret:csye6225-db-password-*"
      }
    ]
  })
}
...
resource "aws_iam_role_policy_attachment" "attach_secretsmanager_policy" {
  role       = aws_iam_role.ec2_combined_role.name
  policy_arn = aws_iam_policy.secretsmanager_read_access.arn
}